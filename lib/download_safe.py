...
def download_with_resume(
    ...
    proxy: Optional[str] = None,      # NEW: proxy support
    ...
    - Hỗ trợ proxy (http, https, socks5).
    
    Args:
        proxy: URL proxy, ví dụ:
               - "http://user:pass@host:port"
               - "socks5://user:pass@host:port"
               - None để không dùng proxy
    """
    ...
    while attempt < max_retries:
        ...
        try:
            ...
            # Mở client và stream với proxy
            with httpx.Client(
                headers=headers,
                follow_redirects=True,
                http2=True,
                timeout=httpx.Timeout(timeout_s),
                proxy=proxy,  # httpx nhận trực tiếp proxy string
            ) as client:
            ...
            if verify_size and os.path.exists(tmp_path):
                ...
                if_total = _guess_total_size(url, headers, final_size, proxy) if total is None else total
...
def _guess_total_size(url: str, headers: dict, current_size: int, proxy: Optional[str] = None) -> Optional[int]:
    """
...
if __name__ == "__main__":
    ...
    url = "https://huggingface.co/datasets/raymondt/tram_yeu/blob/main/39754.jpg"
    url = url.replace("blob/main", "resolve/main")
    output_path = "39754.jpg"
    
    # Ví dụ với proxy
    proxy = "http://oxft0s2j:oXfT0s2J@193.39.184.117:56789"
    
    # Không dùng proxy
    # ok = download_with_resume(url, output_path, referer=None, cookie=None)
    
    # Dùng proxy
    ok = download_with_resume(url, output_path, referer=None, cookie=None, proxy=proxy)
    print("Done:", ok)
